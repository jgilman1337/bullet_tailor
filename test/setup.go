...
//
// If a variable is missing, it falls back to bullet_tailor defaults.
func InitGPTConfig(t *testing.T) (*bullet_tailor.GPTAuth, *bullet_tailor.GPTParams) {
	t.Helper()

	// Define the default auth and param structs
	auth := bullet_tailor.DefaultGPTAuth()
	params := bullet_tailor.DefaultGPTParams()
